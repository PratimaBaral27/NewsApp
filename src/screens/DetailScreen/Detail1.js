...
const Detail1 = (props) =>{
    ...
    const image = {uri : (data && data[1].featured_image_urls.full[0]) }
            ...
            <ScrollView>
                ...
                <FontAwesome5 name={'arrow-left'}  style = {styles.icons} size = {25} color = 'white' onPress = {() => Actions.landing()}></FontAwesome5>
                   <TouchableOpacity style = {styles.button}>
                       <Text style = {styles.buttonText}>{data && data[1].type}</Text>
                   </TouchableOpacity>

                  <Text style = {styles.textTwo}>{data && data[1].title.rendered}</Text> 
                 <Text style = {styles.textThree}>{data && data[1].author_info.display_name}</Text>
                 <Text style = {styles.textFour}>{data && data[1].date}</Text> 
                 </ImageBackground> 
                
                <HTMLView value = {data && data[1].content.rendered} style= {styles.content} />
             </ScrollView>
...
export default newsApi(Detail1);